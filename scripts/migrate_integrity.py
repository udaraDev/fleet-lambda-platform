    ...
    for batch in fetch_all("SELECT batch_id,rows_valid FROM pipeline_batches WHERE archive_manifest IS NULL ORDER BY batch_id"):
        ...
        with connection() as conn, conn.cursor() as cur:
            for item in manifest["files"]:
                path = DATA_DIR / "raw" / str(batch["batch_id"]) / item["path"]
                for chunk in pq.ParquetFile(path).iter_batches(batch_size=1000):
                    ...
                    execute_values(cur, """INSERT INTO stream_event_keys
                        ...
                        (e["event_id"], fingerprint(e), e["event_ts"], e["vehicle_id"], e["trip_id"], batch["batch_id"])
